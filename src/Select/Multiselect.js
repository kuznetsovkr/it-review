...
export default class Multiselect extends Component{
    ...
    async getOptions() {
        const response = await $authHost.get('http://192.168.99.100:8080/api/class')
    ...
    handleSubmit = (e) => {
        ...
        const info = {beginDate: this.state.beginDate, endDate: this.state.endDate, minuses: this.state.minuses,
            ...
            employed: this.state.employed, date: '2022-05-22', mark:  Number(this.state.mark), classId: this.state.value, userId: this.state.userId}
        console.log(info)
        $authHost.post(`http://192.168.99.100:8080/api/review`,  info )
            .then(res => {
                console.log(info)
            })
